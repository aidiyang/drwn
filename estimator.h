...
class UKF : public Estimator {
    public:
        UKF(mjModel *m, mjData * d,
                double _alpha, double _beta, double _kappa, double _noise, bool debug = false) : Estimator(m, d) {
...
#pragma omp parallel for
            for (int i=1; i<=L; i++) {
                ...
                //mju_copy(sigma_states[i]->qacc, d->acc, nv);
                ...
                //mju_copy(sigma_states[i+L]->acc, d->acc, nv);
            }
...
#pragma omp parallel for
            for (int i=1; i<=L; i++) {
                ...
                mju_copy(sigma_states[i]->qpos,   &x[i](0), nq);
                mju_copy(sigma_states[i]->qvel,   &x[i](nq), nv);
                //mju_copy(sigma_states[i]->acc, d->acc, nv);
                mju_copy(sigma_states[i+L]->qpos, &x[i+L](0), nq);
                mju_copy(sigma_states[i+L]->qvel, &x[i+L](nq), nv);
                //mju_copy(sigma_states[i+L]->acc, d->acc, nv);
            }
