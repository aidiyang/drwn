...
int CM730::OneTxRxPacket(unsigned char *txpacket, unsigned char *rxpacket, int priority)
{
	...
	if(length < (MAXNUM_TXPARAM + 6))
	{
        ...
        start_time = m_Platform->GetCurrentTime();
		if(m_Platform->WritePort(txpacket, length) == length)
		{
            write_time = m_Platform->GetCurrentTime();
			if (txpacket[ID] != ID_BROADCAST)
			{
				...
				read_time = m_Platform->GetCurrentTime();
				parse_time = m_Platform->GetCurrentTime();
			...
			else if(txpacket[INSTRUCTION] == INST_BULK_READ)
			{
				...
				m_Platform->SetPacketTimeout(to_length*2);
...
double CM730::GetCurrentTime()
{
    return m_Platform->GetCurrentTime();
}

void Robot::CM730::Sleep(double msec)
{
    m_Platform->Sleep(msec);
}
