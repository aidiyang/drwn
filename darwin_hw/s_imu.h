...
class PhidgetIMU {
    ...
    public:
        ...
        PhidgetIMU(int delay, bool zero_gyro, double imu_alpha) { // delay in ms, or negative to disable handler
            imu_ok = true;
            //imu = NULL;
        ...
        bool getData(double *a, double *g) {
            ...
            a[0] = accel[0] + alpha * (r[0] - accel[0]); 
            a[1] = accel[1] + alpha * (r[1] - accel[1]);
            a[2] = accel[2] + alpha * (r[2] - accel[2]);
            ...
            r[0] = 0.017453*r[2]; // watch the reordering
            r[1] = 0.017453*r[1];
            r[2] = -0.017453*r[0];

            g[0] = gyro[0] + alpha * (r[0] - gyro[0]); 
            g[1] = gyro[1] + alpha * (r[1] - gyro[1]);
            g[2] = gyro[2] + alpha * (r[2] - gyro[2]);
