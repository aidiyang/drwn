...
void mouse_move(GLFWwindow * window, double xpos, double ypos)
{
  ...
  if (perturb) {
    if (selbody > 0)
      mjv_moveObject(action, dx, dy, &cam.pose, (float)width, (float)height, refpos, refquat);
  }
  // camera control
  else
...
void advance(void)
{
  ...
  if (selbody > 0) {
    ...
    // movable object: set mouse perturbation
      ...
      mjv_mousePerturb(m, d, selbody, perturb, refpos, refquat, d->xfrc_applied + 6 * selbody);
  }
  // advance simulation
  mj_step(m, d);
...
// render
void render(GLFWwindow * window, std::vector<mjData *> frame, bool render_inplace)
{
  ...
  //mjr_render(0, rect, &objects2, &ropt, &cam.pose, &con);
  ...
  if (showdepth) {
    ...
    for (int r = 0; r < rect.height; r += 4)
      for (int c = 0; c < rect.width; c += 4) {
        ...
        // assign rgb
        depth_rgb[3 * adr] = depth_rgb[3 * adr + 1] = depth_rgb[3 * adr + 2] = (unsigned char)((1.0f - depth_buffer[r * rect.width + c]) * 255.0f);
      }
  ...
  if (showinfo) {
    if (paused)
      ...
      mjr_overlay(rect, mjGRID_BOTTOMLEFT, 0, "Time\nSize\nCPU\nFPS\nEngy\nStat\nCam", status, &con);
  }
